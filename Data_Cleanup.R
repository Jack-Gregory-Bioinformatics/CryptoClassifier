...
for (file in csv_files) {
  ...
  data <- data %>%
    select(Label, Mean, Circ., Area, Feret) %>%
    rename(
      Condition = Label,
  ...
  #data <- data %>%
  #  separate(Label, into = c("Date", "Strain", "Condition", "Time", "Stain", "Rep", "Image", "Cell"), sep = "_") # nolint
...
# Extract and format the Condition column
combined_data$Condition <- str_sub(combined_data$Condition, 1, 3)  # Take the first 3 characters # nolint
